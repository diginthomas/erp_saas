    ...
    <div :class="[wrapperClasses, 'relative']">
      ...
      >
        <Icon
          ...
          :icon="icon"
          class="mr-2 size-4 text-current"
        />

        <ISpinner
          v-if="$refs.uploadRef && $refs.uploadRef.active"
          class="-mt-1 mr-2 inline-flex size-4 text-current"
        />
      ...
      <div class="ml-2 flex items-center space-x-2">
        <slot name="upload-button" :upload="$refs.uploadRef">
          <button
            ...
            class="inline-flex cursor-pointer items-center rounded-full bg-primary-50 px-4 py-1.5 text-sm font-medium text-primary-800 hover:bg-primary-100 focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2 focus:ring-offset-primary-50 disabled:pointer-events-none disabled:opacity-60"
            @click="$refs.uploadRef.active = true"
          >
            <Icon icon="CloudArrowUp" class="mr-2 size-4 text-current" />
            {{ uploadButtonText }}
          </button>
        </slot>
        ...
        <button
          ...
          v-t="'core::app.cancel'"
          type="button"
          class="inline-flex items-center rounded-full bg-danger-50 px-4 py-1.5 text-sm font-medium text-danger-800 hover:bg-danger-100 focus:outline-none focus:ring-2 focus:ring-danger-600 focus:ring-offset-2 focus:ring-offset-danger-50"
        ...
        <button
          ...
          v-t="'core::app.clear'"
          type="button"
          class="inline-flex items-center rounded-full bg-neutral-50 px-4 py-1.5 text-sm font-medium text-neutral-800 hover:bg-neutral-100 focus:outline-none focus:ring-2 focus:ring-neutral-600 focus:ring-offset-2 focus:ring-offset-neutral-50"
          @click="clear"
        />
      </div>
    </div>
...
const props = defineProps({
  ...
  styleClasses: {
    type: [Object, Array, String],
    default:
      '!flex items-center rounded-full px-4 py-1.5 text-sm font-medium bg-primary-50 text-primary-800 hover:bg-primary-100 focus:ring-offset-primary-50 focus:ring-primary-600 focus:outline-none focus:ring-2 focus:ring-offset-2 cursor-pointer',
  },
})
