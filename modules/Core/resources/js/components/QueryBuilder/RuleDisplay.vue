<template>
  <span
    v-if="index > 0"
    class="rounded-md border border-neutral-300 px-3 py-[0.4rem] text-neutral-600 dark:border-neutral-500 dark:text-neutral-300"
    v-text="$t('core::filters.conditions.' + condition)"
  />

  <div
    v-if="rule.type === 'rule' && rule.query.rule"
    class="shrink-0 snap-end rounded-md border border-neutral-300 px-3 py-[0.4rem] text-neutral-500 dark:border-neutral-500 dark:text-neutral-300"
  >
...
const query = computed(() => {
  return props.rule.query
})

const original = computed(() => {
  const originalObject = find(
    store.state['filters'].availableRules[props.identifier],
    ['id', query.value.rule]
  )
...
function valueLabelWhenDate() {
  if (query.value.operator === 'is' || query.value.operator === 'was') {
    let operatorOptions = original.value.operatorsOptions[query.value.operator]
