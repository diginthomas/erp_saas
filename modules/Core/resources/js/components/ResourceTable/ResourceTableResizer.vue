...
function setHeadingsWidth() {
  ...
  for (let i = 0; i < headings.length; i++) {
    let currentWidth = props.columns[i].width

    headings[i].style.width = currentWidth !== 'auto' ? currentWidth : 'auto'
  }
...
function createGripDiv() {
  ...
  div.classList.add(
    ...
    'dark:group-hover/th:bg-neutral-500'
  )
...
function init() {
  ...
  for (let i = 0; i < totalHeadings; i++) {
    ...
    if (!props.columns[i].customizeable) {
      continue
    }
