<template>
  ...
  <div
    :class="!showEmptyState && initialDataLoaded ? 'opacity-100' : 'opacity-0'"
  >
    <div
      class="mb-3 space-y-1 md:flex md:items-center md:space-x-4 md:space-y-0"
    >
      <SearchInput
    ...
    <IOverlay :show="isLoading">
      <div
        :class="[
          'mt-3 block overflow-hidden rounded-lg border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-900',
          isSticky ? 'border-t-0' : '',
        ]"
      >
        ...
        <ITable
          ...
          :bordered-inner="tableSettings.bordered === true ? true : 'y'"
          fixed-layout
        >
          <ResourceTableResizer
          ...
          >
            <SortableDraggable
              ...
              v-bind="{
                ...
                filter: '.resizer, .draggable-exclude, .table-primary-column',
          ...
          <tbody>
            <tr
              ...
              :key="row.id"
              :ariaSelected="row.tSelected"
              :class="['group/tr', row._row_class]"
...
import ResourceTableResizer from './ResourceTableResizer.vue'
...
async function request(viaUserSearch = false) {
  ...
  try {
    ...
    emit('loaded', { isEmpty: isEmpty.value })
  } catch (error) {
    console.error(error)
  } finally {
    setLoading(false)
...
function saveColumns(columns, refreshTable = true) {
  if (!scriptConfig('demo')) {
    ...
    Innoclapps.info(
      'The visibility state on the column is not saved in the demo and will reset upon refreshing the page.',
      7000
    )
...
th.sortable-chosen.sortable-column-ghost {
  @apply ring-1 ring-inset ring-neutral-400 dark:ring-neutral-500;
}
</style>
